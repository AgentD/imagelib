...
//
//----------------------------------------------------------------------------
CImage::CImage( ) : m_imageBuffer(NULL), m_width(0), m_height(0), m_depth(0),
                    m_type(EIT_NONE)
{
}
...
CImage::E_LOAD_RESULT CImage::load( const std::string& filename,
                                    E_IMAGE_FILE type )
{
...
//
//----------------------------------------------------------------------------
void CImage::allocateBuffer( size_t width, size_t height, size_t depth,
                             E_IMAGE_TYPE type )
{
    if( m_imageBuffer )
       free( m_imageBuffer );
...
void CImage::setPixel( size_t x, size_t y, size_t z,
                       unsigned char R, unsigned char G, unsigned char B )
{
    if( x>=m_width || y>=m_height || z>=m_depth || !m_imageBuffer ||
        m_type==EIT_NONE )
        return;
...
//
//----------------------------------------------------------------------------
void CImage::printf( size_t x, size_t y, size_t z,
                     unsigned char R, unsigned char G, unsigned char B,
                     const char* formatstring, ... )
{
    ...
    for( size_t i=0; buffer[i]!='\0'; ++i )
    {
        ...
        getCharacter( buffer[i], char_buffer, 0, 0, cw, ch );
